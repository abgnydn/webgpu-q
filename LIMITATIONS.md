...
| Open-shell DFT (UKS) SCF | **Shipped 2026-05 (LDA only)** — `runUKSDFT(integrals, nAlpha, nBeta, nucleiSymbols, opts)` in `src/chemistry/dft/uks-scf.ts`. Mirror of `rks-scf.ts` on spin-resolved densities D_α / D_β with separate Fock matrices F_σ = h + J(D_total) + V_xc^σ − ½ hfMix · K(D_σ). Uses `evalXC_LSDA` from functional-spin.ts for the spin-polarized kernel. Stacked-spin DIIS, symmetry-breaking initial guess (0.01 Ha default), per-spin level shift. ⟨S²⟩ via Mayer formula (same as UHF). Validated: closed-shell H₂O UKS-LDA matches RKS-LDA to ≤ 1e-6 Ha; H atom doublet ⟨S²⟩ = 0.75 exactly; Li doublet cc-pVDZ converges with non-zero spin density. GGA UKS (BVWN5/BLYP/B3VWN5/B3LYP5) — needs spin-polarized `buildVxcGGA` with cross-spin γ_αβ derivatives — refuses with clear message; queued follow-up. UKS-TDDFT response α(ω) (closes the last cell of the matrix) is a downstream follow-up after GGA UKS. | shipped ✓ (LDA) |
