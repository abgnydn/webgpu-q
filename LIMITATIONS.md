...
| Frequency-dependent polarizability α(ω) (TDHF / RPA) | **Shipped 2026-05**. `tdhfPolarizability(hf, integrals, shells, ω)` in `src/chemistry/tdhf.ts`. Closed-shell RHF only. Solves the symmetric RPA response equation `[(A−B)(A+B) − ω²·I] X = (A−B)·F` and contracts `α(ω)_μν = 4·X^μ·F^ν`. At ω → 0 reproduces `cphfPolarizability` to 1e-7. Monotonic dispersion below the first pole verified on H₂O STO-3G. Unlocks dynamic polarizability spectra, C₆ dispersion coefficients (Casimir-Polder integral over imaginary axis), refractive index. Open-shell TDHF (UHF reference) is queued. | shipped ✓ |
