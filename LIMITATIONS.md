...
| Open-shell TDHF α(ω) for radicals | **Shipped 2026-05**. `uhfTdhfPolarizability(uhf, integrals, shells, ω)` and `uhfTdhfPolarizabilityImag(...)` in `src/chemistry/uhf-tdhf.ts`. UHF reference, 4-spin-block (A±B) on the combined (α-OV + β-OV) space. Refactored `uhf-cphf.ts` to expose `buildUHFCPHFHessians` (cross-spin block of A−B = 0 since cross-spin A = B; same-spin block carries exchange). Validation: static limit matches `uhfCphfPolarizability` to 1e-7; closed-shell limit (n_α=n_β H₂O) matches RHF TDHF α(ω) to 1e-7 at ω = 0.1 Ha; α(iω) for Li doublet cc-pVDZ decreases monotonically. Unlocks radical-dimer C₆ via open-shell Casimir-Polder (next). | shipped ✓ |
| C₆ van-der-Waals dispersion coefficients | **Shipped 2026-05**. `c6Coefficient(hfA, intsA, shellsA, hfB, intsB, shellsB, opts?)` in `src/chemistry/dispersion.ts` (closed-shell convenience) and `c6CoefficientGeneral(srcA, srcB, opts?)` (any combination of RHF / UHF references via an `AlphaImagSource` discriminated union). Casimir-Polder integral C₆(A,B) = (3/π)·∫₀^∞ ᾱ_A(iω)·ᾱ_B(iω) dω with α(iω) via `tdhfPolarizabilityImag` or `uhfTdhfPolarizabilityImag`. Gauss-Legendre quadrature (default 16 points) with the ω = ω₀·(1+u)/(1−u) substitution mapping [0,∞)→[−1,1]; Golub-Welsch Jacobi-tridiagonal eigensolver for the nodes/weights. Quadrature converges to ≤ 2% from N=8 → N=16 → N=32. Symmetric C₆(A,B) = C₆(B,A) to 1e-10. BeH₂ > H₂ self-dispersion holds. Closed-shell-via-UHF (n_α=n_β H₂) C₆ matches RHF route to 1e-6. Li doublet self-C₆ via open-shell route finite + positive. Absolute magnitudes need aug-cc-pVDZ; STO-3G underestimates α(0) by ~2× (so C₆ ~4× too small). | shipped ✓ |
