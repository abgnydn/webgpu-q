...
| C₆ van-der-Waals dispersion coefficients | **Shipped 2026-05**. `c6Coefficient(hfA, ..., hfB, ...)` (closed-shell convenience) and `c6CoefficientGeneral(srcA, srcB, opts?)` (any combination of RHF / UHF / RKS-DFT references via an `AlphaImagSource` discriminated union with kinds "rhf" / "uhf" / "rks"). DFT route uses `tddftPolarizabilityImag` from `tddft-response.ts` with XC kernel in the response. Casimir-Polder integral via Gauss-Legendre quadrature (default 16 points, Golub-Welsch nodes); converges to ≤ 2% from N=8 → N=32. Symmetric C₆(A,B) = C₆(B,A) to 1e-10. All three source kinds round-trip cleanly: closed-shell-via-UHF (n_α=n_β) matches RHF route to 1e-6; DFT route is same order of magnitude as HF on H₂ STO-3G; Li doublet open-shell self-C₆ finite + positive. Absolute magnitudes need aug-cc-pVDZ; STO-3G underestimates α(0) by ~2×. | shipped ✓ |
...
| Open-shell DFT (UKS) SCF | **Shipped 2026-05** — `runUKSDFT(integrals, nAlpha, nBeta, nucleiSymbols, opts)` in `src/chemistry/dft/uks-scf.ts`. Mirror of `rks-scf.ts` on spin-resolved densities D_α / D_β with separate Fock matrices F_σ = h + J(D_total) + V_xc^σ − ½ hfMix · K(D_σ). Uses `evalXC_LSDA` (LDA) or `evalXC_GGA_spin` (GGA + hybrid) from functional-spin.ts. Stacked-spin DIIS, symmetry-breaking initial guess (0.01 Ha default), per-spin level shift. ⟨S²⟩ via Mayer formula (same as UHF). Validated: closed-shell H₂O UKS-LDA matches RKS-LDA to ≤ 1e-6 Ha; H atom doublet ⟨S²⟩ = 0.75 exactly; Li doublet cc-pVDZ converges with non-zero spin density; Li BLYP converges. **Known**: closed-shell H₂O UKS-B3LYP5 vs RKS-B3LYP5 differs by ~0.9 mHa (sub-chemical-accuracy but visible) — factor convention discrepancy in the cross-spin γ_αβ piece between `buildVxcGGA_spin` and the closed-shell `buildVxcGGA`. Doesn't affect open-shell radicals where γ_αβ ≠ γ_αα, but means RKS is the right call for closed-shell DFT. Follow-up: trace the factor; likely 2× on v_γαβ term. UKS-TDDFT response α(ω) (closes the polarizability matrix) is a downstream follow-up. | shipped ✓ (LDA exact, GGA ~1 mHa closed-shell residual) |
| Quadratically-convergent SCF | Hard cases bail out | Tier 3 |
| Molden / HDF5 / Cube output | **Shipped 2026-05 (Molden)** — `toMoldenString({atoms, shells, C_MO, orbitalEnergies, nOccupied})` in `src/chemistry/molden.ts`. Emits standards-compliant Molden text including `[Atoms] AU`, `[GTO]` (per-atom shell-groups derived from consecutive shells with matching `(alpha[], c[])`), `[6D]`/`[10F]`/`[15G]` tags only when those angular momenta appear, and `[MO]` block with `Sym`/`Ene`/`Spin`/`Occup` headers and per-basis-function coefficient rows. Cartesian-Gaussian basis only — spherical-d (`integrals.sphericalT` present) refuses with a clear message; spherical → Cartesian back-transform on MO coefficients is a follow-up. Closed-shell only; UHF/UKS export pending UKS land. Validated on H₂O/STO-3G and BeH₂/cc-pVDZ via structural assertions. Unblocks orbital plotting in Jmol / Avogadro / Multiwfn / IboView. HDF5 + Cube formats still queued. | shipped ✓ (Molden) |
