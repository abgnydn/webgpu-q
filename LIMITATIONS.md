...
|---|---|---|
| Counterpoise / BSSE (HF + MP2 + CCSD) | **Shipped 2026-05**. `runCounterpoise(atoms, fragments, basis, opts, method, ccsdOpts)` in `src/chemistry/counterpoise.ts`. Boys-Bernardi: per-fragment energy in the full dimer basis via ghost atoms (`ghost?: boolean` flag on `Atom` — basis present, Z=0, no electrons). `method` selects HF / MP2 / CCSD; the ghost-atom plumbing passes through to the post-HF correlation step unchanged. Returns supermolecule energy, per-fragment energies in dimer + bare basis, uncorrected + CP interaction energies, and `bsseCorrection = ΔE_CP − ΔE ≥ 0`. Verified by 7 tests in `counterpoise.test.ts` (HF + MP2 + CCSD on H₂...H₂). Open-shell counterpoise (UHF/UCCSD) — analogous mirror, not yet wired. | shipped ✓ |
...
| Frequency-dependent polarizability α(ω) (TDHF / RPA) | **Shipped 2026-05**. `tdhfPolarizability(hf, integrals, shells, ω)` in `src/chemistry/tdhf.ts`. Closed-shell RHF only. Solves the symmetric RPA response equation `[(A−B)(A+B) − ω²·I] X = (A−B)·F` and contracts `α(ω)_μν = 4·X^μ·F^ν`. At ω → 0 reproduces `cphfPolarizability` to 1e-7. Monotonic dispersion below the first pole verified on H₂O STO-3G. Open-shell TDHF (UHF reference) is queued. | shipped ✓ |
| C₆ van-der-Waals dispersion coefficients | **Shipped 2026-05**. `c6Coefficient(hfA, intsA, shellsA, hfB, intsB, shellsB, opts?)` in `src/chemistry/dispersion.ts`. Casimir-Polder integral C₆(A,B) = (3/π)·∫₀^∞ ᾱ_A(iω)·ᾱ_B(iω) dω with α(iω) via `tdhfPolarizabilityImag`. Gauss-Legendre quadrature (default 16 points) with the ω = ω₀·(1+u)/(1−u) substitution mapping [0,∞)→[−1,1]; Golub-Welsch Jacobi-tridiagonal eigensolver for the nodes/weights. Quadrature converges to ≤ 2% from N=8 → N=16 → N=32. Symmetric C₆(A,B) = C₆(B,A) to 1e-10. BeH₂ > H₂ self-dispersion (more electrons → bigger α → bigger C₆) holds. Absolute magnitudes need cc-pVDZ+aug-cc-pVDZ; STO-3G underestimates α(0) by ~2× (so C₆ ~4× too small). | shipped ✓ |
