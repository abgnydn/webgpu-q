...
| Frozen-core in IP-EOM / EA-EOM-CCSD | `runIP_EOMCCSD` and `runEA_EOMCCSD` not yet wired — same pattern as `runEOMCCSD` applies (restrict the 1h / 1p / 2h1p / 1h2p packed bases to non-frozen occupied indices). | Tier 3 — ~1 day; mechanical mirror of the EE-EOM frozen-core port. |
