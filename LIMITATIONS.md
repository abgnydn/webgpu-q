...
| Foster-Boys orbital localization | **Shipped 2026-05** — `fosterBoys(C_MO, integrals, shells, nOccupied, opts?)` in `src/chemistry/foster-boys.ts`. Boys 1960 maximization of Σ_i ⟨ψ_i\|r\|ψ_i⟩² via 2×2 Jacobi sweeps. Returns unitary rotation matrix U (n_occ × n_occ) and per-sweep L history. Localized orbitals = C_MO[:, :nOcc] · U. Converges in 5-20 sweeps for typical small molecules. Validated: H₂O HF/STO-3G L monotonically increases over 7 sweeps to ~3.04; U is orthogonal to 1e-9 on CH₄; localization actually moves L away from canonical-MO value (≥ 0.5 lower bound). Useful for chemistry interpretation: identifies σ-bonds, lone pairs, core orbitals as individual localized MOs. Pipek-Mezey (Mulliken-based) localization is a follow-up. | shipped ✓ |
