...
dispatch ceiling for sequential workloads and a
...
simulation is a third domain, and, as we show, one that sits far closer to the
bandwidth-bound end of that spectrum: each gate already moves the entire
$2^N$-amplitude statevector, so the per-dispatch overhead is a much smaller
fraction of the work than in those deeply dispatch-bound workloads, and
dense-tile fusion additionally crosses into a compute-bound regime
(\S\ref{sec:tierladder}). The same technique that delivers hundreds-fold
speedups there therefore caps at a few-fold here. Locating where a workload sits
on that dispatch-bound-to-bandwidth-bound spectrum is the point. The work runs
...
at $k=32$ reach 2.62$\times$ ($N=14$, depth 160). It is tempting to read the
fused path's \emph{logical} effective throughput --- the statevector traffic of
all $D$ gates, $16\,D\,2^N/t$, which rises to 866\,GB/s at $N=20$, depth 160 ---
as memory-bandwidth saturation. It is not: that figure exceeds this device's
$\approx$200\,GB/s DRAM bandwidth by $4.3\times$ precisely \emph{because} fusion
is working --- a $k=32$ chain reads each amplitude from DRAM once, applies all
32 gates in registers, and writes once, so it does the work of 866\,GB/s of
naive per-gate traffic while moving only one read+write pass per fused dispatch.
The \emph{physical} DRAM bandwidth, $16\lceil D/k\rceil 2^N/t$, peaks at just
$\sim$27\,GB/s --- \textbf{13.5\% of the device bound} --- so the fused kernel is
\emph{not} bandwidth-saturated in this regime. The tell is in the artifact:
the fused wall-time is pinned at $\sim$3\,ms across $N=10$ to $N=20$ (a
$1000\times$ range of state sizes), i.e.\ floor-limited by the harness sync
fence rather than by memory traffic. The fused-vs-unfused speedup compresses
toward 1.25$\times$ at low depth ($N=20$, depth 40) because few dispatches remain
to remove, not because bandwidth is saturated; at longer depth the dispatch term
still dominates and the gain persists ($N=20$, depth 160 stays $\sim$2.6$\times$).
True DRAM saturation is the asymptotic ceiling fusion drives \emph{toward} as
$N$ grows past the floor regime; demonstrating it empirically --- at statevector
sizes where the per-pass compute time dwarfs the $\sim$3\,ms fence --- is future
work (\S\ref{sec:fusionlimits}). All figures here are derived from the committed
\texttt{E10-throughput} artifact's $(N,D,k,t)$ fields.

\section{Honest limitations}
\label{sec:fusionlimits}

\begin{itemize}
  \item \textbf{Tier-D plateau (\S\ref{sec:tierladder})} --- fusion past
  three-qubit tiles is compute-bound and does not reach the 5$\times$ target.
  Committed as a \texttt{status:"fail"} artifact with the compute-vs-memory
  diagnosis.
  \item \textbf{Bandwidth saturation is asserted asymptotically, not measured.}
  At the widths we test ($N\le20$) the fused wall-time is floor-limited by the
  harness sync fence ($\sim$3\,ms, constant across a $1000\times$ range of state
  sizes), so physical DRAM utilization peaks at only $\sim$13.5\% of the device
  bound. We therefore do \emph{not} claim the fused kernel saturates memory
  bandwidth in the measured regime; establishing the true asymptotic ceiling
  needs statevector
  sizes large enough that per-pass compute dwarfs the fence ($N\gtrsim24$), a
...
Per-dispatch overhead is the dominant cost for fine-grained sequential GPU
work, which is exactly what fusion targets. Native GPU statevector simulators --- NVIDIA
...
end of the spectrum --- each statevector gate already moves the full amplitude
array, so the workload is far less dispatch-bound to begin with, and dense-tile
fusion crosses into compute-bound at Tier-D (\S\ref{sec:tierladder}) --- so
fusion's payoff is bounded at a few-fold, and that plateau marks the crossover
explicitly.
