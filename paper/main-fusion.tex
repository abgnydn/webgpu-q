...
fusion saves. Fusion is correctness-preserving (360/360 test cells pass, worst
...
the regime where fusion stops paying off. The work runs entirely in a browser
tab.
...
C --- $N{=}15$, depth $80$; D --- $N{=}14$, depth $80$. \normalsize
...
\begin{figure}[ht]
  ...
  is compute-bound --- $\sim$256 complex multiplies per amplitude, a
  $4\times$ growth over Tier-C, while the memory traffic fusion eliminates
  grows only linearly. The plateau is committed as a \texttt{status:"fail"}
  artifact, not hidden.}
  \label{fig:ladder}
\end{figure}
...
because the four-qubit dense block performs $\sim$256 complex multiplies per
amplitude --- a 4$\times$ growth over Tier-C --- while the memory traffic that
fusion eliminates grows only linearly. Tier-D crosses out of the
...
and the fused-vs-unfused speedup compresses toward 1.25$\times$ at $N=20$ ---
exactly the expected crossover, since once a kernel saturates memory bandwidth
there is no dispatch overhead left to remove. The goal of fusion is to reach
