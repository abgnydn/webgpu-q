...
integration on every push, not by aggregate test counts. The end-to-end browser
benchmarks driven through headless Chromium are run locally rather than in CI,
because hosted runners expose no WebGPU adapter; the GPU-dependent timings
reported here are therefore reproduced on the author's hardware and are not
protected by an automated regression gate. The EOM-CCSD family
