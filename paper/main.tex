...
(CCSD(T) vs FCI) otherwise. Where it overlaps native suites (PySCF, Psi4,
ORCA) it is 2--6$\times$ slower; its contribution is not speed but delivery
--- full quantum chemistry reachable from a URL.
\end{abstract}
...
tabs and devices. Yet no published system runs \emph{electronic structure} in
the browser, and none distributes such a calculation across browser tabs.
...
exploits the $K[\mu,\nu] = K[\nu,\mu]$ symmetry; the X-build inner loop is
$4\times$-unrolled. Workers are pre-warmed before the SCF loop so iteration 1
...
\paragraph{Setup (once).} The master computes the full B-tensor, partitions the
auxiliary index range $[0, n_{\mathrm{aux}})$ into $N$ disjoint slices, and
ships slice $T$ to worker tab $T$. Each worker acknowledges receipt; the master
then holds only its own slice. (A per-tab \emph{independent} build --- each tab
forming only its aux-slice from scratch, so the full B never resides on any
single tab --- is designed but not yet implemented; \S\ref{sec:limitations}.)
...
(f32 reduction noise, six orders below chemical accuracy of 1.594\,mHa). The
distributed swarm Fock build matches the single-tab reference to
$\lesssim 10^{-12}$\,Ha end-to-end (benzene, naphthalene).

Correctness is enforced continuously: the repository carries 845+ unit tests
(run on every push under continuous integration) plus a suite of end-to-end
browser benchmarks driven through headless Chromium. The EOM-CCSD family is
gated by \emph{full-tensor} brute-force diagnostics --- the complete
...
partitioning was verified to reproduce the single-slab Fock build to relative
error $\lesssim 10^{-15}$ at 2-, 3-, 4-, and 8-way splits before any multi-tab
run.
...
exploit K symmetry                     & 20\,s $\to$ 17.6\,s \\
$4\times$ SIMD unroll on X-build       & iters 4--13 $-17\%$ \\
\bottomrule
\end{tabular}
\end{center}

End-to-end naphthalene HF: \textbf{77\,s $\to$ 28\,s} single-tab
($2.7\times$) before any swarm.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.88\textwidth]{fig-optimization.pdf}
  \caption{Cumulative single-tab kernel optimization on naphthalene cc-pVDZ.
  Reusing per-thread JK scratch buffers across SCF iterations and exploiting
  the $K[\mu,\nu]=K[\nu,\mu]$ symmetry are the two largest wins; the $4\times$
  SIMD X-build unroll removes a further $\sim$17\% on the JK-dominated
  iterations. End-to-end HF wall falls $77\,\mathrm{s}\to28\,\mathrm{s}$
  ($2.7\times$) before any swarm parallelism.}
...
\toprule
molecule & basis & $n$ & $n_{\mathrm{aux}}$ & path & wall & $E$ (Ha) \\
\midrule
benzene      & cc-pVDZ & 120 & 662  & 2-tab               & 5.5\,s & $-230.72269$ \\
naphthalene  & cc-pVDZ & 190 & 1048 & 4-tab $\times$ 2-inner & 14\,s  & $-383.38458$ \\
anthracene   & STO-3G  & 80  & 708  & 4-tab               & 8\,s   & $-526.92320$ \\
pentacene    & STO-3G  & 124 & 1091 & 4-tab $\times$ 2-inner & 38\,s  & $-821.63271$ \\
\textbf{\Csixty{}} & STO-3G & 300 & 2520 & 4-tab $\times$ 2-inner & 730\,s & $\mathbf{-2244.10176}$ \\
\bottomrule
\end{tabular}
\end{center}

The naphthalene swarm (14\,s) is faster than the optimized single-tab parallel
path (28\,s) because four independent V8 processes incur less coordination
...
\begin{figure}[ht]
  ...
  \Csixty{}, colored by basis set, log-time axis. The swarm reaches \Csixty{}
  (300 basis functions); the open marker shows naphthalene's single-tab time
  (28\,s) above its swarm time (14\,s), a $2\times$ intra-machine gain. Note
  the single-tab SharedArrayBuffer ceiling is basis-dependent ($\sim n=220$ for
  cc-pVDZ, higher for STO-3G), so it is stated in text rather than drawn as one
  line.}
  \label{fig:scaling}
\end{figure}
...
\begin{itemize}
  ...
  real win.
  \item \textbf{\Csixty{} runs only at STO-3G}; cc-pVDZ at $n\approx600$
  exceeds even the swarm-distributed master's build memory.
\end{itemize}
...
target~\cite{webgpudispatch}. The intersection --- in-browser electronic
structure, and browser-tab distribution of it --- is, to our knowledge,
unpublished. Best-functional and benchmark context follows the GMTKN55
...
artifacts are the source of every number in this paper.
...
2--6$\times$ slower; its value lies elsewhere --- in removing the install,
hardware, and cost barriers between a learner and a real calculation, and in
showing that the browser's memory ceiling is not a hard limit but one that a
tab swarm can raise. The entire artifact is a URL, and a referee can re-run
