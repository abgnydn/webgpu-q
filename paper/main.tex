...
algorithm is identical, and to $\le 0.5$\,mHa (HF; $\le 0.1$\,mHa with
spherical-d) / $\le 0.25$\,mHa (CCSD(T) vs FCI) otherwise. Speed is workload-dependent and is not the
...
but the calculation runs server-side on a native backend. Open Computational
Chemistry (OCC)~\cite{occ} has recently shown Hartree--Fock and DFT running
client-side in the browser via WebAssembly. No published system, however, brings
the \emph{correlated post-Hartree--Fock} hierarchy --- MP2 through CCSD(T) and
EOM-CCSD --- into the browser, accelerates the perturbative-triples bottleneck
with WebGPU, or distributes such a calculation across browser tabs
(\S\ref{sec:related}).
...
integration on every push, not by aggregate test counts. Continuous integration
covers the unit suite, type checking and linting; a subset of the distributed
browser-tab specs additionally runs on pull requests that touch that subsystem,
since those require only \texttt{BroadcastChannel} and \texttt{SharedArrayBuffer}.
The \emph{GPU-dependent} end-to-end benchmarks driven through headless Chromium
are run locally rather than in CI, because hosted runners expose no WebGPU
adapter; the GPU timings reported here are therefore reproduced on the author's
hardware and are not protected by an automated regression gate. The EOM-CCSD family
...
\begin{itemize}
  ...
  \item \textbf{No GPU speedup for the density-fitted Fock build.} The
  aux-basis 3-index DF the swarm relies on runs on the f64
  WebAssembly path. WebGPU exposes no f64, so the f32 GPU DF kernels cannot
  accelerate the f64-bound $J$/$K$ contraction; they are retained only as a
  $\sim$30\,mHa screening path. A real GPU win awaits double-single (df64)
  emulation of the JK products.
...
electronic structure. WebGPU per-dispatch overhead is itself an actively
characterized quantity~\cite{webgpudispatch}. Web-based quantum-chemistry tools
do exist ---
...
while the SCF runs on a native backend (GAMESS, Gaussian, etc.). The closest
prior client-side work is Open Computational Chemistry (OCC)~\cite{occ}, which
runs Hartree--Fock and DFT in-browser via WebAssembly as a single worker.
webgpu-q instead runs the entire HF/MP2/CCSD(T)/DFT/EOM stack \emph{in} the
browser, client-side, with no backend. On a literature search current to
June~2026 we find no published in-browser (client-side) electronic-structure
implementation
...
because \texttt{queue.submit} is non-blocking. For the GPU-kernel experiments,
5 warmup samples are discarded and 20 retained, reporting
median/p10/p90/p99/std/IQR. Experiments that deviate from this protocol --- most
notably the E34 wall-clock comparison against PySCF, which uses a single
un-warmed run per cell --- disclose the deviation in \texttt{meta.timingNote},
and their derived ratios should be read as order-of-magnitude rather than
precise. Failing configurations
