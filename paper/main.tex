...
End-to-end, the distributed-swarm and single-tab SCF energies agree to within
the $10^{-7}$\,Ha convergence tolerance (benzene, naphthalene); the underlying
partitioned Fock build reproduces the single-slab build far more tightly
(below the $10^{-12}$ pass bar, next paragraph).
...
($\approx 2.5\times$); the optimized single-tab end-to-end HF runs in
$\approx 28$\,s, which the 4-tab swarm halves to 14\,s (\S\ref{sec:scaling}).
(A $4\times$ unroll of the SIMD inner loop was also attempted and reverted ---
17.88 vs 18.01\,s on benzene, within $\pm1\%$ noise --- since the 2-lane f64
SIMD already saturates the pipeline at these problem sizes.)
...
\begin{figure}[ht]
  ...
  ($\approx 2.5\times$). The optimized single-tab end-to-end HF runs in
  $\approx 28$\,s; the 4-tab swarm halves that to 14\,s
  (\S\ref{sec:scaling}).}
...
\midrule
benzene      & cc-pVDZ & 120 & 662  & 2-tab               & 5.5\,s & $-230.72273$ \\
...
scaling figures (\S\ref{sec:scaling}) are produced by the end-to-end browser
benchmark specs under \texttt{e2e/} (each spec fixes the molecule, basis, and
tab topology and asserts convergence), and we are extending those specs to emit
the same environment-stamped JSON artifact per run.
