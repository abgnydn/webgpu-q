...
per-phase breakdown. Benzene, anthracene, naphthalene, and pentacene artifacts
are committed (M2 Pro); naphthalene also reproduces on the CI runner. \Csixty{}
is from a local high-memory run --- its $\sim$3\,GB build exceeds the CI
runner's $\sim$2\,GB WASM-heap cap, so a CI-reproducible \Csixty{} artifact
awaits the streaming-kernel rewrite noted in \S\ref{sec:limitations}.
\normalsize
