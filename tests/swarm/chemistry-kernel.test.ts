...
describe("Swarm chem-energy kernel", () => {
  test("HF on H₂ STO-3G at r=0.7414 Å gives the expected energy", () => {
    ...
    const r = runChemEnergyTile(tile);
  ...
  test("Bond-scan energies are smooth (no sudden jumps near minimum)", () => {
    const tiles = bondScanTiles({
      atomA: "H", atomB: "H", basis: "sto-3g",
      rMin: 0.6, rMax: 1.0, nPoints: 9,
    });
    const energies = tiles.map((t) => runChemEnergyTile(t).energy);
